...
\begin{lemma}[正交投影]\label{lem:projection}
设 \( W \) 是内积空间 \( V \) 的有限维子空间，则对任意 \( \mathbf{v} \in V \)，存在唯一的 \( \mathbf{w} \in W \)，使得 \( \mathbf{v} - \mathbf{w} \perp W \)。此时 \( \mathbf{w} \) 称为 \( \mathbf{v} \) 在 \( W \) 上的正交投影。
\end{lemma}
...
\begin{theorem}[秩-零度定理]\label{thm:rank_nullity}
设 \( A \) 是 \( m \times n \) 矩阵，则 \( \text{rank}(A) + \text{nullity}(A) = n \)，其中 \( \text{nullity}(A) = \dim(\ker(A)) \)。
\end{theorem}
